...
export default function InsuranceCompanyList({
 ...
 <Sparkles className="text-[#1A5D40]" fill="#1A5D40" />
 마법같은 보험사 혜택 비교
 </h3>
 ...
 <span key={tag} className="text-[13px] font-bold bg-[#1A5D40]/10 text-[#1A5D40] px-2 py-1 rounded-xl">
 #{tag}
 </span>
 ))}
 </div>
 ...
 <div className="text-[15px] text-slate-400 font-extrabold uppercase tracking-wider mb-0.5">
 최대 꿀 할인율
 </div>
 <div className="text-[27px] font-black text-[#1A5D40] drop-">
 ...
 <div className="w-8 h-8 rounded-full bg-[#1A5D40]/10 flex items-center justify-center shrink-0">
 <Info size={16} className="text-[#1A5D40]" />
 </div>
 ...
 <div className="text-[14px] font-black text-slate-400 uppercase tracking-tight">
 다음 갱신 예상가
 </div>
 <div className="flex flex-col">
 <div className="flex items-center gap-2">
 <span className="text-[15px] font-bold text-slate-300 line-through">
 {formatCurrency(company.basePremium)}
 </span>
 <span className="text-[13px] font-black text-white bg-red-500 px-1.5 py-0.5 rounded-lg">
 -{formatPercent(company.discountRate)}
 </span>
 </div>
 <div className="text-3xl font-black text-slate-900 leading-none mt-1 group-hover:text-[#1A5D40] transition-colors">
 {formatCurrency(company.expectedPremium)}
 </div>
 </div>
 </div>

 <button className="w-12 h-12 bg-slate-900 text-white flex items-center justify-center hover:bg-[#1A5D40] rounded-[20px] transition-colors group-hover:scale-110">
 ...
 <p className="text-[#1A5D40] text-sm font-extrabold flex items-center gap-1 mt-1">
 ...
 isSelected
 ? 'border-[#1A5D40] bg-white ring-8 ring-[#1A5D40]/10 scale-105 z-10'
 : 'border-slate-100 bg-white hover:border-slate-200 opacity-60 hover:opacity-100'
 }`}
 >
 <div className={`text-sm font-black mb-1 ${isSelected ? 'text-[#1A5D40]' : 'text-slate-400'}`}>
 {p.label}
 </div>
 ...
 <ShieldCheck className="text-[#1A5D40]" fill="#143D60" size={28} />
 {activeModalPlan === 'BASIC' ? '기본형' : activeModalPlan === 'STANDARD' ? '표준형' : '프리미엄형'} 보장 내역 팡팡!
 </h5>

 {isLoadingCoverages ? (
 <div className="py-12 text-center text-[#1A5D40] font-black animate-pulse">보장 내역 요정 부르는 중... ✨</div>
 ) : coverageError ? (
 ...
 >
 <div>
 <div className={`text-xs font-black mb-1 ${isIncluded ? 'text-[#1A5D40]' : 'text-slate-400'}`}>
 {cov.category}
 </div>
 <div className="flex items-center gap-2">
 ...
 {isIncluded ? (
 <div className="text-[11px] font-black text-[#1A5D40] bg-[#1A5D40]/10 px-2 py-0.5 rounded-full inline-block mt-1">포함됨</div>
 ) : (
 ...
 className="flex-[2] py-5 bg-[#1A5D40] text-white rounded-[28px] font-black hover:bg-[#143D60] hover:-translate-y-1 transition-all text-lg"
 >
 할인 듬뿍 받고 가입하기!
 </button>
 </div>

...
export default function InsuranceSummaryCard({
 ...
 className="w-full mx-auto rounded-[32px] overflow-hidden relative bg-white"
 >
 <div className="relative z-20 px-6 py-8">
 
 <div className="bg-white/90 backdrop-blur-sm p-6 rounded-2xl flex justify-between items-center ">
 <div>
 ...
 <div className="text-2xl sm:text-3xl font-black text-[#1A5D40]">
 ...
 <CheckCircle2 size={18} className="text-[#1A5D40]" />
 할인 조건 팡팡 현황
 </h4>
 ...
 <span className="text-[16px] font-bold text-slate-700">안전 점수 80점 초과</span>
 <span className={`text-[13.5px] font-black px-2.5 py-1 rounded-xl ${
 currentSummary.safetyScore == null ? 'text-slate-500 bg-slate-200'
 : currentSummary.safetyScore >= 80 ? 'text-[#1A5D40] bg-white border border-[#1A5D40]/30'
 ...
 <div className="flex justify-between items-center bg-white/70 backdrop-blur-sm p-4 rounded-3xl border-[0.5px] border-white ">
 <span className="text-[16px] font-bold text-slate-700">연간 주행 1.5만km 이하</span>
 <span className={`text-[13.5px] font-black px-2.5 py-1 rounded-xl ${
 currentSummary.annualMileageKm === 0
 ? 'text-slate-500 bg-slate-200'
 : currentSummary.annualMileageKm <= 15000
 ? 'text-[#1A5D40] bg-white border border-[#1A5D40]/30'
 : 'text-red-600 bg-white border border-red-200'
 }`}>
 {currentSummary.annualMileageKm === 0 
 ? '달린 기록 없음' 
 ...
 <div className="md:col-span-2 bg-white rounded-[32px] p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
 <div className="text-center md:text-left relative z-10 w-full">
 <div className="text-[15px] text-slate-500 font-extrabold mb-1">
 다음 갱신 시 슈퍼 예상 보험료
 </div>
 ...
 <span className="text-sm font-black text-[#1A5D40] bg-[#1A5D40]/10 px-2 py-0.5 rounded-xl border border-[#1A5D40]/20 ">
 (-{formatPercent(currentSummary.expectedDiscountRate)})
 </span>
 </div>
 <p className="text-[14px] text-slate-500 mt-4 font-extrabold">
 현재까지 누적 절감액 요정님{" "}
 <span className="text-[#1A5D40] ml-1 bg-[#1A5D40]/10 px-2 py-1 rounded-md">{formatCurrency(currentSummary.totalExpectedSavings)}</span>
 </p>
 </div>
